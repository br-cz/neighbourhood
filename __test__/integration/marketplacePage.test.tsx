...
jest.mock('@/src/hooks/eventsCustomHooks', () => {
  const useFetchEvents = () => {
    const [events, setEvents] = useState(mockListings.events);

    useEffect(() => {
      setEvents(mockListings.events);
    }, [mockListings]);

    return { events, refetch: () => setEvents([...mockListings.events]) };
  };

  const useCreateEvent = () => ({
    handleCreateEvent: jest.fn((newEvent) => {
      mockListings.events = [...mockListings.events, newEvent];
...
describe('MarketplacePage Integration Tests', () => {
  test('Creating a listing updates the marketplace feed', async () => {
    renderComponent();
  ...
  test('Creating an invalid listing does not update the marketplace feed', async () => {
    renderComponent();
